...
def basic_experiment(x_train, y_train, x_test, y_test, formatted_print=False):
    ...
    # ====== YOUR CODE: ======
    id3_tree = ID3(label_names=[0, 1])
    id3_tree.fit(x_train, y_train)
    acc = accuracy(y_test, id3_tree.predict(x_test))
    # ========================
...
def best_m_test(x_train, y_train, x_test, y_test, min_for_pruning):
    ...
    # ====== YOUR CODE: ======
    id3_prun_tree = ID3(label_names=[0, 1], min_for_pruning=min_for_pruning)
    id3_prun_tree.fit(x_train, y_train)
    acc = accuracy(y_test, id3_prun_tree.predict(x_test))
    # ========================
...
if __name__ == '__main__':
    ...
    """
    plot_graphs = True
    best_m = 50 # as requested in the assignment - no bonus
